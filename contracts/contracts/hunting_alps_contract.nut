this.hunting_alps_contract <- this.inherit("scripts/contracts/contract", {
		...
		this.m.States.push({
			...
			function start()
			{
				this.Contract.m.BulletpointsObjectives = [
					"在夜晚终结 " + this.Contract.m.Home.getName() + "的梦魇"
				];
	...
	function createScreens()
	{
		...
		this.m.Screens.push({
			...
			Text = "[img]gfx/ui/events/event_79.png[/img]{%SPEECH_ON%请在今晚终结梦魇，你感兴趣吗？%SPEECH_OFF%}",
			Image = "",
			List = [],
			ShowEmployer = true,
			ShowDifficulty = true,
			Options = [
				{
					Text = "{你能为此支付多少克朗? | 让我们谈谈报酬. | 让我们谈谈能得到多少克朗. | 我们可以调查一下，只要你出合适的价格.}",
					function getResult()
					{
						return "Negotiation";
					}
				...
				{
					Text = "{这听起来不像是有利可图的工作. | 这听起来不像是我们的工作. | 这不是我们要找的那种工作.}",
					function getResult()
					{
		...
		this.m.Screens.push({
			...
			Text = "[img]gfx/ui/events/event_79.png[/img]{有村民提醒你要注意小心怪物}",
			Image = "",
			List = [],
			Options = [
				{
					Text = "{睁大你们的眼睛. | 我们需要为此做好准备 | 兄弟们，保持警戒.}",
					function getResult()
					{
						return 0;
					}
		...
		this.m.Screens.push({
			...
			Text = "[img]gfx/ui/events/event_102.png[/img]{你突然看到一个黑色的身影在黑暗中跳跃。它以飞快的速度向前猛扑。它就像一条蛇，但你能听到有人在睡梦中发出窒息的咆哮。你命令士兵拿起武器！}",
			Image = "",
			List = [],
			Options = [
				{
		...
		this.m.Screens.push({
			ID = "Victory",
			Title = "战斗之后...",
			//Text = "[img]gfx/ui/events/event_122.png[/img]{怪物被杀死了。你拿起你的剑砍下怪物的脖子。刀轻松地穿过，脑袋滚落在草地上。它的眼睛是秃的和凹的。里面什么也没有，没有肉，没有肌肉。无论如何，你告诉人们准备好返回找%employer%. | The alps lay in the grass and though you know you saw them hurt, their flesh seems to have healed over and they seem more slain by your resilience than any weapon. You take your sword to saw a head off, only to find the blade slides right through the skin and the neckhole puckers closed. You stab the body a few times, twisting the blade to rend the flesh unmendable. Sinews slither briefly before coming to rest in the wound\'s hole. Not sure what to make of that, you shovel the head into a bag and tell the men ready a return to %employer%.}",
			Text = "[img]gfx/ui/events/event_122.png[/img]{怪物被杀死了。你拿起你的剑砍下怪物的脖子。刀轻松地穿过，脑袋滚落在草地上。它的眼睛是秃的和凹的。里面什么也没有，没有肉，没有肌肉。无论如何，你告诉人们准备好返回找%employer%.}",
			Image = "",
			List = [],
			Options = [
				{
		...
		this.m.Screens.push({
			...
			Text = "[img]gfx/ui/events/event_79.png[/img]{%employer%要求查看梦魇的尸体，确认过后，%employer%答应付款}",
			Image = "",
			Characters = [],
			List = [],
			ShowEmployer = true,
		...
		this.m.Screens.push({
			ID = "GoodNightsSleep",
			Title = "当你返回...",
			Text = "[img]gfx/ui/events/event_33.png[/img]{你敢打赌，这些人已经在%townname%为自己赢得了良好的休息。这些人睡得很沉，就像死了一样。男人们醒来后伸懒腰打了个哈欠。不是每个人都有美梦或噩梦，但短暂的遗忘，一个非常必要.}",
			Image = "",
			List = [],
			Characters = [],
			Options = [
